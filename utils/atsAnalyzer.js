...
const extractKeywordsFromJobDescription = (text) => {
  ...
  words.forEach(word => {
    // filter out short words, stop words, and numbers
    if (word.length >= 3 && !STOP_WORDS.has(word) && isNaN(word)) {
      candidates.add(word);
    }
  });

  // Return up to 15 unique keywords or default if empty
...
const analyzeResume = (text, jobDescription = '') => {
  ...
  // Phone regex (support common phone formats)
  const phoneRegex = /(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g;
  const hasPhone = phoneRegex.test(text);
  if (hasPhone) contactScore += 5;

  // Professional profile URLs (LinkedIn, GitHub or portfolio)
  ...
  const hasEducation = educationKeywords.some(keyword => {
    // Search for whole-word keyword patterns
  ...
  // Keyword Score calculation: fraction of keywords matched, times 20
  let keywordScore = 0;
